...
def getSpanLabelCLS():
    ...
    labelcls = set()
    with open('./alldata.txt', 'r', encoding='utf-8') as f:  # 新做出的BIES标注且check后的数据，准备做TCNER数据的格式转换
    ...
    with open('./mid_data_new/span_ent2id.json', 'w') as f:  # √：14
        f.write(span_ent2id)
    ...
    with open('./mid_data_new/crf_ent2id.json', 'w') as f:  # √：57
        f.write(crf_ent2id)
...
preprocess("./alldata.txt", './mid_data_new', "train")  # 本质上是tc要求的 stack 文件；
